...
Algorithm \ref{img71:methodTypeInference2} contains definitions of three inferences used in the first and second phases.
...
Algorithm \ref{img72:methodTypeInference3} shows remaining two inferences, adding types to lower and upper bounds.
...
fn InferLower(U, V):
  ...
  switch {
	...
	V isLike 'C<$V_1,...,V_e$>' $\rightarrow$
	  temp = ([U] + U.inheritedTypes + U.implementedTypes)
...
fn InferUpper(U, V):
  ...
  switch {
	...
	U isLike 'C<$U_1,...,U_e$>' $\rightarrow$
	  temp = ([V] + V.inheritedTypes + V.implementedTypes)
...
\par
\info{Fix}
The last part of this algorithm is type variable fixation, shown in algorithm  \ref{img73:methodTypeInference4}.
...
We show the W algorithm in schema \ref{img74:w} since it is closely related to C\# and Rust type inference.
