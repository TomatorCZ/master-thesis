...
The \texttt{Generated} folder contains generated nodes of \textit{bound tree}, and the \texttt{Symbols} folder contains symbols for namespaces, classes, methods, etc. that are exposed by the compiler.
...
We didn’t change the Roslyn parser which transformers the source codes into AST. 
...
Replacing the \texttt{MethodTypeInference.cs} file with the \texttt{TypeInference.cs} was the biggest source code update in the commit. 
...
\begin{enumerate}
  \item State -- It contains fields representing the inner state of the current type inference. 
It includes a list of type variables, collected bounds, dependencies between the type variables, already fixed type variables, and type information received from the argument list, the type parameter constraints, the target type, and the type argument list.
  \item Public API -- It contains an entry point for the type inference of a method invocation and an object creation expression.
  \item Phases -- It contains methods corresponding to the first phase and the second phase of our changed method type inference algorithm mentioned in Section \ref{sect:phasesAlg}.
  \item Bounds -- It contains a mechanism for adding a new bound to a type variable, including the bound propagation mentioned in Section \ref{sect:Collection}.
  \item Dependencies -- It contains initialization and update of the defined dependencies (Section \ref{sect:dependencies}. and \ref{sect02:MTIA}) between type variables, and helper functions for querying them.
  \item Input types -- It contains helper functions for getting information about input types (Section \ref{sect02:MTIA}).
  \item Output types -- It contains helper functions for getting information about output types (Section \ref {sect02:MTIA}).
  \item Output type inferences -- It provides an output type inference of our algorithm (Section \ref{sect:FuncTI}).
  \item Explicit type inferences -- It provides an explicit type inference of our algorithm (Section \ref{sect:FuncTI}).
  \item Exact inferences -- It provides an exact type inference of our algorithm.
  \item Lower-bound inferences -- It provides an lower-bound type inference of our algorithm (Section \ref{sect:Collection}).
  \item Upper-bound inferences -- It provides an upper-bound type inference of our algorithm (Section \ref{sect:Collection}).
  \item Return type inferences -- It provides an return type inference of our algorithm (Section \ref{sect:FuncTI}).
  \item Fixing -  It provides a type variable fixation of our algorithm (Section \ref{final:fixation}).
  \item OtherHelpers - It contains unsorted helper methods used by the implementation.
\end{enumerate}
We added the public API, which adapts the old method type inference API to the new one. 
