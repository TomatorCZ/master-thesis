...
\info{Intro}
C\# method type inference is a restricted Hindley-Millner type inference to able to applied in its C\# type system.
Since type inferences in other languages like Rust or Haskell are based on the same principle, we present high-level overview of Hindley-Millner type inference and it's type system to reason about possible extensions of current C\# type inference.
...
We start with describing set of expressions, which will be a target of type inference described in video series \cite{online:HMVideos}.
Expression is either a variable (\ref{expr01}), a function application (\ref{expr02}), a lambda function (\ref{expr03}), or let-in clause (\ref{expr04}).
\par
\begin{align}
e =&\ x\label{expr01}\\
...
\end{align}
\par
Above mentioned expressions have one of two different types.
...
The second group is \textit{poly} types which assembles from a type possible preceding $\forall$ operator \ref{type04} bounding its type variables.
\par
\begin{align}
...
\end{align}
To be able to reason about more complex type expression, a context(\texttt{$\Gamma$}) give us assumptions of a current scope where we evaluates the type of expression.
It contains of pairs of expression and its type from which we can make typing judgements (using \texttt{$\vdash$} operator). 
\par
\info{Set of rules}
H-M deduction system gives us the following inference rules allowing to deduce type of an expression based on assumption given in the context.
The syntax of rule corresponds with what we can judge (the right side of $\vdash$) from the left side of $\vdash$ below the line based on assumptions given above the line.
The rules can be divided into two kinds.
The first four rules give us a manual what types can we expect applying the mention expressions of Lambda calculus.
The two last rules allowing us to specify poly types to mono types and vice-versa.
\begin{align*}
...
With definitions given above, we can start to talk about H-M type inference which is able to find type of every expression of completely untyped program.
There exist several algorithms for inferring most general type of the expression.
We show \textit{W} algorithm in schema \ref{alg05:w} since it is closely related to C\# and Rust type inference.
Inputs are the context $\Gamma$ and an expression which type we want to infer.
The process consists of systematic traversing the expression from bottom to up and deducing type of sub-expressions following the mentioned rules.
The algorithm uses the \texttt{Instantiate} method replacing quantified type variables in the expression to new type variables, the \texttt{Generelize} method to replace free type variables in the expression to quantified type variables, and the \texttt{Unify} method also known as \textit{Unification} in Logic.
Unification is a algorithm finding a substitution which application on the unifying types make them identical.
Outputs of this algorithm is the inferred type with a substitution used for algorithm internal state.
\par
\begin{algorithm}
\SetKwProg{Fn}{fn}{:}{}
...
Relations between this algorithm and C\#, Rust type inference will be discussed in detail later, although we can notice that the unification part of this algorithm is the same as Method type inference mentioned in C\# section where the substitution represents inferred type arguments of the method which parameters were unified with matching arguments.
\par
...
\par
Basic principle of extending H-M type inference by subtyping is described in Parreaux work \cite{paper:Parreaux}, where instead of accumulating type equivalent constrains, we accumulate and propagate subtyping constrains.
These subtyping constrains assemble a set of types, which have to be inherited by the constrained type variable or the variable has to inherit them.
\par
\info{Restriction and possible extensions - overloading}
Extending H-M type inference by supporting overloading mentions Andreas Stadelmeier and Martin Plumicke work \cite{paper:Overloading}.
An important thought behind this paper is to accumulate two types of type variable constraint sets.
Constraints observed from a method call is added into one \textit{AND-set}.
When the method call has multiple overloads the AND-sets are added into \textit{OR-set}.
After accumulating these sets, all combinations of items in OR-sets are generated and solved by type inference.
As we can see, for each method overload participating in type inference, we have to do an alternative type inference containing constrains obtained only from the overloaded method excluding other overloads.
This algorithm can be improved by excluding overloads which can't be used in the method call to save the branching.
