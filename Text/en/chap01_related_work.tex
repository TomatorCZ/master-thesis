...
\info{Intro}
The last subsection mentions C\# developer's environment, where everybody is free to share his/her thoughts about possible improvement or issues with C\# language or Roslyn compiler.
We base our solution on issues mentioned in this section and recommended future possible features by C\# developers in already mentioned C\# language and Roslyn repository.
\par
\info{Proposal}
The process of designing a new language feature starts with publishing an idea into discussions \cite{online:discussions} where is commented by other C\# community.
The idea contains a brief description of the feature, motivation and design.
Besides the idea, a new language feature requires a proposal, initially published as an issue, describing the feature in the way which can be later reviewed by \ac{LDM} committee.
If the proposal sufficiently merits the discussions, it can be mark as a \textit{champion} by a member of \ac{LDM} marking it for being discussed further by the team.
There are several state which the proposal can have.
The most important for us is \textit{AnyTime} meaning that the proposal is not actively worked on and are open to community.
At the time of writing, a member of \ac{LDM} recommended championed issue \cite{online:champion} for being investigated since it contains many related discussions with proposed changes but still doesn't have a required proposal.
When a proposal has sufficient quality to be discussed by \ac{LDM}, a member invites the proposer to make a \textit{Pull Request} where the further collaboration continue.
If \ac{LDM} accepts the proposal, it is added into the \textit{proposals} folder in the repository for being shipped with the next C\# versions and implemented in Roslyn.  
\par
\info{Discussions}
Regarding mentioned issue, we present related discussions directly or indirectly referred from it.
\par
\info{Discussion - Default type parameters}
The first discussion \cite{online:DefTypeParam} mentions \textit{Default type parameters} introducing default type arguments which are used when a explicit type arguments are not used.
Figure \ref{img59:defTypeParam} shows a potential design of this feature where construing generic type \texttt{A} doesn't need a type argument since is used \texttt{int} type as a default value.
\par
\begin{figure}
\begin{lstlisting}[style=csharp]
var temp = new A();
...
The next discussion \cite{online:GenAlias} mentions \textit{Generic aliases} allowing to specify default values similar to the goal of previous discussion by defining an alias to that type with option generic parameters.
We can see an example of usage in Figure \ref{img60:genAlias} where we predefine the first type argument of \texttt{Dictionary} class to be the \texttt{string} type which simplifies usage of that type in scenarios where we find values in dictionary often by string keys.
\par
\begin{figure}
\begin{lstlisting}[style=csharp]
...
Basic thought of this idea is be able to specify a type parameter for which we provide a type argument by name.
...
Using the named type arguments, we can just tell the compiler specific type parameters for which we provide type arguments, \texttt{U} in this case, and let the compiler infer the rest of the type parameters.
\par
\begin{figure}
\begin{lstlisting}[style=csharp]
var x = F<U:short>(1);
...
Comments of mentioned championed issue \cite{online:champion} proposes several keywords which can be used in a type argument list for skipping type arguments which can be inferred by the compiler and just providing the remaining ones.
...
The comments propose other options for keywords like an underscore or whitespaces. 
\par
\begin{figure}
\begin{lstlisting}[style=csharp]
Foo<var, int>("string");
...
Discussion \cite{online:RetTInference} proposes \textit{Target-typed inference} where type inference uses type information of target where return value is assigned.
We can see the usage in Figure \ref{img62:RetTInf} where type inference determines that return type has to be \texttt{int} type and use that to deduce the type argument \texttt{T}.
\par
\begin{figure}
\begin{lstlisting}[style=csharp]
object row = ...
...
We can see the usage in following Figure \ref{img64:TMRetInf} where return type of method \texttt{Add} is inferred to be \texttt{int} based on type of the return expression.
\par
\begin{figure}
\begin{lstlisting}[style=csharp]
...
Issue \cite{online:Realloc} proposes a way how to compact type argument lists of identifier containing sub identifiers with argument lists.
We can see an idea demonstrated in example \ref{img65:Realloc} where the argument list of \texttt{A<T1>} type and the \texttt{Foo<T2>} method are merged and the type arguments split by semicolon. 
\par
\begin{figure}
\begin{lstlisting}[style=csharp]
A.Foo<int;string>();
...
\end{figure}
\par
\change{Discussion - Constructor type inference}
...
The type inference can be seen in Figure \ref{img66:CtorTInf} where \texttt{T} type parameter of \texttt{C<T>} generic type can be deduced by using type information from its constructor.
\par
\begin{figure}
\begin{lstlisting}[style=csharp]
