...
The compilation pipeline, described in \cite{online:roslynArchitecture}, starts with loading \texttt{.csproj} file with related C\# sources (\texttt{.cs}) and referenced libraries (\texttt{.dll}) as we can see in the figure \ref{img56:roslynPip}.
C\# sources are passed to the lexer creating tokens used by the parser forming \ac{AST}.
Constructing \ac{AST} is the first phase (green color boxes of the figure \ref{img56:roslynPip}) of the compiler checking syntax of C\# sources.
\par
\info{Describe the second phase}
The second phase, marked by orange color, forms Named symbols exposed by public API representing defined namespaces, classes, methods and etc. in the C\# project.
The declarations are received from C\# sources by traversing \ac{AST} and seeking for the particular syntax.
Libraries are stored in \texttt{.dll} format which are parsed by \texttt{MetadataLoader} creating the same Named symbols as that received from C\# sources.
\par
\info{Describe the third phase}
The third, binding, phase matches identifiers in the code with received Named symbols from the previous phase.
Because processing of method body is not dependent on other method bodies since the code only using already known declarations, Roslyn makes this phase concurrent.
The result of the phase is \emph{BoundTree} where all identifiers refer to the Named symbols.
A method binding itself is a complicated procedure consisting of many subtasks as overload resolution or mentioned method type inference which algorithm we described in detail in the previous section.
\par
\info{Binder}
The binding is divided into chain of binders taking care of smaller code scopes.
One purpose of the binders is ability to resolve an identifier to the Named symbol, if the referred symbol lies in their scope.
If they can't find the symbol, they ask to the preceding binder.
This process is called \emph{LookUp} determining which Named symbols are accessible in the current position.
Examples of binder are \texttt{NamespaceBinder} resolving defined top level entities in the namespace scope, \texttt{ClassBinder} resolving defined class members, or \texttt{MethodBinder} binding method bodies.
The last mentioned binder sequentially iterates body statements and matches identifiers with their declarations.
Statement and expression binding are an important steps which are related to type inference.
The first observation is that statement binding doesn't involve binding of following statements which can be referred as a backward binding.
The consequence is that C\# is not able to infer types in backward direction.
As an example can be usage of \texttt{var} keyword in variable declarations which has to be used always with initializing value.
...
The preceding step before Method Type Inference is Overload resolution being a part of \texttt{MethodCallExpression} binding.
...
So, at time of deciding which method should be called, we have to resolve right version of the methods following language rules for method resolution.
This step involves binding the method call arguments firstly and then deciding which parameter list of the method group fits the argument list the best.
If the method group is generic and the expression doesn't specify any type arguments, Method Type Inference is invoked to determine type arguments of the method before the selection of the best candidate for the call.
\par
\info{NullableWalker}
Method type inference can occur for the second time if we turned on nullability analysis mentioned previously.
Nullability analysis is a kind of Flow analysis which uses Bound tree to check and rewrite already created bound nodes accordingly to nullability.
Because overloading and method type inference is nullable-sensitive the whole binding process is repeated respecting the nullability and reusing results from the previous binding.
The required changes are stored during the analysis and the bound tree is rewrited by the changes at the end of the analysis.
